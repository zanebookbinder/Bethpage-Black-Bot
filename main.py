...
class BethpaigeBlackBot:
    ...
    def __init__(self, clear_table_before_starting):
        ...
        try:
            ...
            print('Exception:', e)
            error_message = traceback.format_exc()  # Get the full stack trace as a string
    ...
    def get_new_tee_times(self):
        ...
        existing_times = dynamo_db_connection.get_all_teetimes()
        if existing_times:
            existing_times = existing_times[0]['data']
        
        # PUBLISH CURRENT TIMES TO DB
        dynamo_db_connection.publish_teetimes(filtered_tee_times)

        # SUBTRACT DB FROM CURRENT SITE
        new_times = self.remove_existing_tee_times(filtered_tee_times, existing_times)
    ...
    def remove_existing_tee_times(self, times_from_site, existing_times):
        ...
        if existing_times:
            # Sort existing times
            existing_times_set = set(tuple(sorted(item.items())) for item in existing_times)

            # Find items on current site that aren't in the existing db table
            new_times = [item for item in times_from_site if tuple(sorted(item.items())) not in existing_times_set]
        else:
...
def lambda_handler(event, context):
    clear_table_before_starting = 'clear_table_before_starting' in event \
        and event['clear_table_before_starting']
    BethpaigeBlackBot(clear_table_before_starting)
    return {"status": "done"}