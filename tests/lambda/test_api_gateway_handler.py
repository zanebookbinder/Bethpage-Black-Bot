...
def make_handler():
    """Create ApiGatewayHandler with mocked dependencies."""
    with patch("api_gateway_handler.EmailSender") as mock_email, \
         patch("api_gateway_handler.DynamoDBConnection") as mock_ddc, \
         patch("api_gateway_handler.OneTimeLinkHandler") as mock_otlh:
        from api_gateway_handler import ApiGatewayHandler
        h = ApiGatewayHandler()
        # Attach mocks to handler for test access
        h._mock_ddc = mock_ddc.return_value
        h._mock_otlh = mock_otlh.return_value
        h._mock_email = mock_email.return_value
        return h
...
class TestOriginValidation:
    ...
    def test_valid_localhost_origin(self):
        ...
        h._mock_ddc.get_latest_tee_times_all.return_value = []
        resp = h.handle(event)
        assert resp["statusCode"] == 200
...
class TestRoutes:
    def test_get_recent_times(self):
        ...
        h._mock_ddc.get_latest_tee_times_all.return_value = [{"Time": "8am"}]
        resp = h.handle(event)
    ...
    def test_register(self):
        ...
        h._mock_ddc.add_email_to_all_emails_list.return_value = (True, "")
        resp = h.handle(event)
        body = json.loads(resp["body"])
    ...
    def test_register_duplicate(self):
        h = make_handler()
        event = make_event("POST", "/register", body={"email": "a@b.com"})
        h._mock_ddc.add_email_to_all_emails_list.return_value = (False, "Email is already in list")
        resp = h.handle(event)
        body = json.loads(resp["body"])
    ...
    def test_get_user_config_found(self):
        ...
        h._mock_ddc.get_user_config.return_value = {"id": "a@b.com", "min_players": 2}
        resp = h.handle(event)
        body = json.loads(resp["body"])
        assert body["success"] is True
    ...
    def test_get_user_config_not_found(self):
        h = make_handler()
        event = make_event("POST", "/getUserConfig", body={"email": "a@b.com"})
        h._mock_ddc.get_user_config.return_value = None
        resp = h.handle(event)
        body = json.loads(resp["body"])
        assert body["success"] is False
    ...
    def test_create_one_time_link(self):
        ...
        h._mock_ddc.get_user_config.return_value = {"id": "a@b.com"}
        resp = h.handle(event)
        body = json.loads(resp["body"])
        assert body["success"] is True
    ...
    def test_validate_one_time_link_valid(self):
        ...
        h._mock_otlh.validate_one_time_link_and_get_email.return_value = (True, "a@b.com")
        resp = h.handle(event)
        body = json.loads(resp["body"])
    ...
    def test_validate_one_time_link_invalid(self):
        ...
        h._mock_otlh.validate_one_time_link_and_get_email.return_value = (False, "Expired")
        resp = h.handle(event)
        body = json.loads(resp["body"])
